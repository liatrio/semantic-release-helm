...
const verifyConditions = async (
    ...
    try {
        ...
        errors.push(new SemanticReleaseError(`Error verifying Helm version: ${error.message}`));
    }

    // verify that `charts` config option is set, and each specified chart passes `helm lint`
    if (!charts || charts.length === 0) {
        errors.push(new SemanticReleaseError("Expected `charts` option to be set with at least one chart path"));
    } else {
        await Promise.all(charts.map(async (chart) => {
            try {
                await helmLint(chart);
            } catch (error) {
                errors.push(new SemanticReleaseError(`Chart ${chart} failed validation: ${error.message}`));
            }
        }));
    }
    ...
    if ((!github && !aws) || (github && aws)) {
        errors.push(new SemanticReleaseError("Expected either `aws` or `github` config options to be set"));
    ...
    if (github) {
        ...
        if (!github.pagesBranch) {
            errors.push(new SemanticReleaseError("Expected `github.pagesBranch` config option to be set"));
        } else {
            ...
            try {
                ...
                errors.push(new SemanticReleaseError(`Error fetching branch "${github.pagesBranch}" for GitHub Pages: ${error.message}`));
            }
        }
        ...
        if (!process.env.GITHUB_TOKEN) {
            errors.push(new SemanticReleaseError("GITHUB_TOKEN environment variable must be set"));
        ...
        try {
            ...
            if (!repository.data.has_pages) {
                errors.push(new SemanticReleaseError(`GitHub Pages is not enabled for repository ${owner}/${repo}`));
            }
        } catch (error) {
            errors.push(new SemanticReleaseError(`Error fetching GitHub repository: ${error.message}`));
        }
    }
    ...
    if (aws) {
        ...
        if (!aws.region || !aws.bucket || !aws.bucketUrl) {
            errors.push(new SemanticReleaseError("Expected `aws.region`, `aws.bucket`, and `aws.bucketUrl` config options to be set"));
        ...
        try {
            ...
            errors.push(new SemanticReleaseError(`Error determining AWS Caller Identity: ${error.message}`));
        ...
        try {
            ...
            errors.push(new SemanticReleaseError(`Error running S3.HeadBucket: ${error.message}`));
        }
    }
